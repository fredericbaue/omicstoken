...
$DefaultMemuraiPath = "C:\Program Files\Memurai\memurai.exe"

$RelativeMemuraiPath = Join-Path $PSScriptRoot "memurai\memurai.exe"



function Get-MemuraiExecutable {

    if (Test-Path $DefaultMemuraiPath) {

        return $DefaultMemuraiPath

    }

    $memuraiCmd = Get-Command memurai.exe -ErrorAction SilentlyContinue

    if ($memuraiCmd -and $memuraiCmd.Source) {

        return $memuraiCmd.Source

    }

    if (Test-Path $RelativeMemuraiPath) {

        return $RelativeMemuraiPath
...
python -c "import fastapi, redis, numpy" 2>$null
...
if ($MemuraiService -and $MemuraiService.Status -eq "Running") {
    ...
    $MemuraiExe = Get-MemuraiExecutable
    if ($MemuraiExe) {
        Write-Host "?? Launching Memurai from $MemuraiExe..."
        $redis = Start-Process -FilePath $MemuraiExe -ArgumentList "--port 6379" -PassThru
        Start-Sleep -Seconds 2
    } else {
        Write-Host "? Could not locate memurai.exe. Checked:" -ForegroundColor Red
        Write-Host "   - $DefaultMemuraiPath" -ForegroundColor Yellow
        Write-Host "   - System PATH (Get-Command)" -ForegroundColor Yellow
        Write-Host "   - $RelativeMemuraiPath" -ForegroundColor Yellow
        Write-Host "   Please install Memurai or update launch.ps1 with the correct path." -ForegroundColor Red
        exit 1
    }
}
...
if ($redis) {
    ...
    Wait-Process -Id $redis.Id,$backend.Id
